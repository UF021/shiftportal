...
def delete_application(
    ...
    a = _get_app(app_id, hr, db)
    # Sever FK references from PreRegistration before deleting
    db.query(models.PreRegistration).filter(
        models.PreRegistration.application_id == app_id
    ).update({models.PreRegistration.application_id: None}, synchronize_session=False)
    db.delete(a)
    db.commit()
