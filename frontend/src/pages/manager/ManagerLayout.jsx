...
const NAV = [
  ...
  { path: '/manager/clock',    icon: '⏱',  label: 'Attendance' },
  { path: '/manager/staff',    icon: '👥', label: 'My Staff'   },
  { path: '/manager/holidays', icon: '🌴', label: 'Holidays'   },
]
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--navy)', color: 'var(--text)' }}>
      ...
      <div style={{
        ...
        <div onClick={() => nav('/manager')} style={{ cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{
            ...
            <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Site Manager</span>
          </div>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 60px)' }}>
        {/* Sidebar */}
        <div style={{
          width: 200, background: 'var(--navy-mid)', borderRight: '1px solid var(--border)',
          ...
          {NAV.map(({ path, icon, label }) => {
            const active = pathname === path || (path !== '/manager' && pathname.startsWith(path))
            return (
